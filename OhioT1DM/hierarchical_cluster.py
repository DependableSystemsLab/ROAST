#!/usr/bin/env python3
"""Paper-faithful two-cluster DTW clustering for OhioT1DM profiles."""
...
LABELS = tuple([f"A_{index}" for index in range(6)] + [f"B_{index}" for index in range(6)])
...
def hierarchical_cluster(risk_profiles_directory, output_directory):
    ...
    if len(profiles) != len(LABELS):
        raise ValueError(f"expected {len(LABELS)} OhioT1DM profiles, found {len(profiles)}")
    result = cluster_risk_profiles(list(zip(LABELS, profiles)))
    ...
    joblib.dump(np.arange(len(LABELS)), output_directory / "AllPatientIDs.pkl")
